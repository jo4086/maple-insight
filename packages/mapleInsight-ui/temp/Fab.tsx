...
export const Fab = (props: FabProps) => {
  ...
  return (
    <div
      ref={fabRef}
      className="Fab_container"
      style={{ top: `${position.y}px, right: ${position.x}px` }}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      <div className="Fab_root" onMouseDown={handleMouseDown}>
        <span>Drag Me!</span>
      </div>
      {isMenuOpen && (
        <ul className="Fab_menu">
          {actions.map((action, index) => (
            <li key={index} className="Fab_action_item" onClick={action.onClick}>
              {action.icon}
