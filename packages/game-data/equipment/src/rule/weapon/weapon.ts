...
type WeaponTypeConstantMap = Record<WeaponConstantTypeLabel, number>;
...
const baseWeaponTypeMetaMap = {
  ...oneHandedWeaponTypeMetaMap,
  ...twoHandedWeaponTypeMetaMap,
} as const satisfies Record<WeaponType, WeaponPartMeta>;

export type WeaponTypeMeta = WeaponPartMeta & {
  /** 무기상수 */
  weaponConstant: number;
};

export const weaponTypeMetaMap = Object.fromEntries(
  weaponTypes.map((key) => [
    key,
    {
      ...baseWeaponTypeMetaMap[key],
      weaponConstant: weaponConstantMap[key],
    },
  ]),
) as {
  [K in WeaponType]: (typeof baseWeaponTypeMetaMap)[K] & { weaponConstant: (typeof weaponConstantMap)[K] };
};
