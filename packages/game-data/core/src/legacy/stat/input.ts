...
/**
 * inputBaseStatMetaMap: 힘/민첩/올스탯/AP/레벨당/HP/MP/DF
 * inputPercentStatMetaMap: 스탯%, AP 스탯%, HP/MP%
 * inputAttackStatMetaMap: 공격력/마력/공마%/순수 무기 공격력 비율/무기 a* 숙련도
 * inputDamageStatMetaMap: 데미지/보공/일몹뎀/상태이상 추가뎀/크뎀/최종뎀
 * inputDefenseStatMetaMap: 방어력/방어력%/방무/몬스터 방어율 감소
 * inputCriticalStatMetaMap: 크확
 * inputCooldownStatMetaMap: 쿨감
 * inputDurationStatMetaMap: 버프/소환수 지속시간
 * inputRewardStatMetaMap: 경험치/메소/드랍
 * inputResistanceStatMetaMap: 속성/상태이상 저항
 * inputMovementStatMetaMap: 이동/점프
 * inputUtilityStatMetaMap: 공속/스탠스/스타포스
 * */
...
export const inputAttackStatMetaMap = {
  ...
  'attack-power-rate': { label: '공격력(%)', aliases: ['attack-power-rate', 'attackPowerRate', 'attRate', 'attackPowerR', 'attackR'] },
  'magic-power-rate': { label: '마력(%)', aliases: ['magic-power-rate', 'magicPowerRate', 'matRate', 'magicPowerR', 'magicR'] },
...
export const inputDamageStatMetaMap = {
  ...
  'boss-damage': { label: '보스 몬스터 데미지(%)', aliases: ['boss-damage', 'bossDamage', 'bossDam', 'bossDamR', 'bossMobDam', 'bossMobDamR'] },
  'normal-damage': {
    label: '일반 몬스터 데미지(%)',
  ...
  'critical-damage': { label: '크리티컬 데미지(%)', aliases: ['critical-damage', 'criticalDamage', 'criDamage', 'criDam', 'criDamR'] },
  'final-damage': { label: '최종 데미지(%)', aliases: ['final-damage', 'finalDamage', 'finalDam', 'finalDamR', 'fd'] },
} as const satisfies InputStatMetaMap;

export const inputDefenseStatMetaMap = {
  armor: { label: '방어력', aliases: ['armor', 'defense'] },
  'armor-rate': { label: '방어력(%)', aliases: ['armor-rate', 'armorRate', 'armorR', 'defense-rate', 'defenseRate', 'defenseR'] },
  'ignore-monster-armor': {
    label: '몬스터 방어율 무시(%)',
    aliases: ['ignore-monster-armor', 'ignoreMonsterArmor', 'ignore-monster-defense', 'ignore-mob-armor', 'ignore-mob-defense', 'ima', 'imd', 'imaR', 'imdR', 'ied'],
  },
  'monster-armor-reduction': {
    label: '몬스터 방어율 감소(%)',
    aliases: ['monster-armor-reduction', 'monsterArmorReduction', 'monster-defense-reduction', 'mob-armor-reduction', 'mob-defense-reduction', 'mar', 'marR', 'mdr', 'mdrR'],
  },
} as const satisfies InputStatMetaMap;

export const inputCriticalStatMetaMap = {
  'critical-rate': { label: '크리티컬 확률(%)', aliases: ['critical-rate', 'criticalRate', 'cri', 'criR', 'critical'] },
} as const satisfies InputStatMetaMap;
...
export const inputDurationStatMetaMap = {
  'buff-duration-rate': { label: '버프 지속시간(%)', aliases: ['buff-duration-rate', 'buffDurationRate', 'buff-duration', 'buffDuration', 'bd', 'bdR'] },
  'summon-duration-rate': { label: '소환수 지속시간(%)', aliases: ['summon-duration-rate', 'summonDurationRate', 'summon-duration', 'summonDuration', 'sd', 'sdR'] },
} as const satisfies InputStatMetaMap;
...
export const inputStatLabelMap = Object.fromEntries(inputStatKeys.map((key) => [key, inputStatMetaMap[key].label])) as {
  [K in InputStatKey]: (typeof inputStatMetaMap)[K]['label'];
};

export const inputStatAliasKeyMap = Object.fromEntries(inputStatKeys.flatMap((key) => inputStatMetaMap[key].aliases.map((alias) => [alias, key]))) as Record<
  InputStatAlias,
  InputStatKey
>;
