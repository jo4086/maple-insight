...
export function getWorkerConfig(env: NodeJS.ProcessEnv = process.env): WorkerConfig {
  try {
    ...
    const pageStart = parsePositiveInt(env.CRAWLER_RANKING_PAGE_START, DEFAULT_PAGE_START);
    const pageEnd = parsePositiveInt(env.CRAWLER_RANKING_PAGE_END, DEFAULT_PAGE_END);

    return {
      mode: parseAppMode(env.APP_MODE ?? env.NODE_ENV),
      apiKey,
      ...
      intervalMs: parsePositiveInt(env.CRAWLER_INTERVAL_MS, DEFAULT_INTERVAL_MS),
      requestTimeoutMs: parsePositiveInt(env.CRAWLER_REQUEST_TIMEOUT_MS, DEFAULT_REQUEST_TIMEOUT_MS),
      requestBatchSize: parsePositiveInt(env.CRAWLER_REQUEST_BATCH_SIZE, DEFAULT_REQUEST_BATCH_SIZE),
      maxFetchCount: parsePositiveInt(env.CRAWLER_MAX_FETCH_COUNT, DEFAULT_MAX_FETCH_COUNT),
    };
  } catch (error) {
