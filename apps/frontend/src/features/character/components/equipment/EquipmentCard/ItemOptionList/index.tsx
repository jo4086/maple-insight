import type { ItemOption, ItemOptionSet } from '@maple/types';
import { Fragment } from 'react/jsx-runtime';

import { optionLabelMap, optionOrder, percentKeys, type ItemOptionKey } from './constants';

const getOptionValue = (option: ItemOption | undefined, key: ItemOptionKey) => {
  return option?.[key] ?? 0;
};

type ItemOptionListProps = {
  options: ItemOptionSet;
};

export const ItemOptionList = ({ options }: ItemOptionListProps) => {
  const { total, starforce, add, base, etc, exceptional } = options;
  ...
  return (
    <>
      {visibleKeys.map((key) => {
        ...
        const etcValue = getOptionValue(etc, key);
  ...
  return (
    <>
      {visibleKeys.map((key) => {
        ...
        ].filter((part) => part.alwaysShow || part.value !== 0);
